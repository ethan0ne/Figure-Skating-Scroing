...
class PreNormResidual(nn.Module):
    ...
    def forward(self, x):
        if self.transpose:
            ...
            out = self.fn(x_t)
            return out.transpose(1, 2) + x_norm
        return self.fn(self.norm(x)) + x

class scoring_head(nn.Module):
    def __init__(self, depth, input_dim, dim, input_len=2, num_scores=1, bidirection=False):
        super().__init__()
        ...
        self.linear_forward = nn.Sequential(
            *[nn.Sequential(
                PreNormResidual(dim, FeedForward(dim, dense = partial(nn.Conv1d, kernel_size=1)), transpose=True),
        ...
        # for c3d & vggish
        self.video_transform_linear = nn.Linear(4096, input_dim)
        self.audio_transform_linear = nn.Linear(768, input_dim)
